---
title: "ltvProject"
author: "Venturers-Group 3: Sixuan Tian, Yi Chen Huang, Ruixin Huang, Anita Nelliat"
output: 
  html_document:
    toc: true
    toc_depth: 3
...
segmented.customers <- as.data.frame(aggregate(segmented.customers, by = list(members), mean))
segmented.customers <- segmented.customers %>% dplyr::rename(Type = Group.1)
...
ggplot(data = df2, aes(x = xq, y = lifespan.norm)) +
  ...
  facet_grid(status.label ~ segment.label)
```
#Based of the observation of the quantile plot, we can notice that our clustering performance is great. 
#Further explanation would be written later

# Predicting via Random Forest
- Using 70% of the `segmented.customers` dataset (with the users segmented into their groups) for training and the remaining 30% as the test set, we run the random forest function with our output variable as `segment`.
