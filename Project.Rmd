...
- Let us try decreasing the segments to 2 to see if more distinct status based segments can be obtained 
```{r}
# Increasing the segments to 2
members = cutree(hierarchical.cluster, k = 2)
aggregate(cancelled.customers.train, by = list(members), mean)
```
- With 2 segments we can see clearer segments forming with distinct behaviour.
