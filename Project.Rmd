...
customer.firstMonth.logins <- ltv.logins  %>%  filter(row_number()==1) %>% select("id", "count")
customer.previousMonth.logins <- ltv.logins  %>%  filter(row_number()==n()-1) %>% select("id",  "count")
customer.lastMonth.logins <- ltv.logins  %>%  filter(row_number()==n()) %>% select("id", "count")
...
for (i in seq(length(customer.overall.means$id))) {
  ...
  customer.overall.means$lastToFirstLoginRatio[i] <- lastMonthLogins/firstMonthLogins
  customer.overall.means$lastToPrevLoginRatio[i] <- lastMonthLogins/prevMonthLogins
}
...
- First Method: predict lifespan of different segments customer
