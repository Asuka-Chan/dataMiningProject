...
cancelled.customers = createDataPartition(customer.overall.means$id[which(customer.overall.means$lastStatus == 2)],l=F, p = 0.7)
# we create a 30% data partition from the subset of customers who are active
active.customers = createDataPartition(customer.overall.means$id[which(customer.overall.means$lastStatus == 1)],l=F, p = 0.3)

customer.train <- rbind(active.customers, cancelled.customers)

scalled.customer = scale(customer.overall.means)
cancelled.customers.train = customer.overall.means[customer.train, ]
cancelled.customers.test = customer.overall.means[-customer.train, ]
...
- Let us try increasing the segments to see if more distinct status based segments can be obtained 
```{r}
# Increasing the segments to 8
members = cutree(hierarchical.cluster, k = 8)
aggregate(cancelled.customers.train, by = list(members), mean)
```
- With 8 segments we can see clearer segments forming with distinct behaviour.
 - In general we notice that as the lifespan increases customers tend to stop/cancel the subscription. This is alarming to the company as it is clear that more effort must be maintained in retaining current subscribers. A clear indication that customers would leave based on increasing lifespan is the difference between the two ratios. Larger the lastToPrevLogin ratio compared to the lastToFirst the more likely that the customer would stop the subscription. We see that group 4, has a big lifespan and their ratio difference is almost 200%. 
